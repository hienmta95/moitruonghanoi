                    ...
                    <section class="prd-detail">
                        <section class="panel-body">
                            <div class="prd-contents">
                                <div class="label"><span>{{ $congnghe[$title] }}</span></div>
                                <div class="content detail-content">
                                    ...
                                    <div>
                                        <i style="font-size: 13px; float: right">
                                            {{ date_format(date_create($congnghe['updated_at']),"d-m-Y") }}</i>
                                    </div>

                                    <!-- social -->
                                    <div class="socials-share">
                                        <a class="bg-facebook" href="https://www.facebook.com/sharer/sharer.php?u={{ route('congnghe', ['id'=>$congnghe['id'], 'slug'=>$congnghe['slug']]) }}" target="_blank">
                                            <span class="fa fa-facebook"></span> Share
                                        </a>
                                        <a class="bg-twitter" href="https://twitter.com/share?text=moitruonghanoi&url={{ route('congnghe', ['id'=>$congnghe['id'], 'slug'=>$congnghe['slug']]) }}" target="_blank">
                                            <span class="fa fa-twitter"></span> Tweet
                                        </a>
                                        <a class="bg-google-plus" href="https://plus.google.com/share?url={{ route('congnghe', ['id'=>$congnghe['id'], 'slug'=>$congnghe['slug']]) }}" target="_blank">
                                            <span class="fa fa-google-plus"></span> Plus
                                        </a>
                                        <a class="bg-pinterest" href="https://www.pinterest.com/pin/create/button/?url={{ route('congnghe', ['id'=>$congnghe['id'], 'slug'=>$congnghe['slug']]) }}&media={{ asset('/').$congnghe['image']['url'] }}&description={{ $congnghe['title'] }}" target="_blank">
                                            <span class="fa fa-pinterest"></span> Pin
                                        </a>

                                        <a class="bg-email" href="https://mail.google.com/mail/u/0/?view=cm&fs=1&to{{ $info['emailcongty'] }}&su={{ $congnghe['title'] }}&body={{ $congnghe['slug'] }}" target="_blank">
